...
import { GetAppName, CLEAN_DATE, GetAge, CleanMessage } from "../../context/App";
...
import { SEARCH_STUDENTS, REMOVE_STUDENT, UPDATE_STUDENT, ADD_GUARDIAN } from "../../queries/Student.query";
...
const StudentList: FC<IProps> = ({ history }) => {
    ...
    const [GetGuardianTypes, { loading: gTLoading }] = useLazyQuery(GET_GUARDIAN_TYPES, {
        onError: (err) => {
            toast.error(CleanMessage(err.message));
                                    ...
                                    <div className="row justify-content-center">
                                        ...
                                        <div className="col-lg-4">
                                            ...
                                            <Select
                                                options={classes}
                                                value={{
                                                    label: searchInput?._class?.name || <span className="text-gray">Select...</span>,
                                        ...
                                        <div className="element-box no-bg bg-white">
                                            <div className="table-responsive">
                                                <h6 className="element-header">
                                                    Returned Students of <b className="text-primary">( {data?.SearchStudents?.docs[0]?.current_class?.level?.name} )</b>
                                                </h6>
                                                <table className="table table-striped">
                                                    ...
                                                    <tbody>
                                                        {data?.SearchStudents?.docs.map((stu: any, index: number) => (
                                                            <tr key={index}>
                                                                <td>{index + 1}</td>
                                                                <td>
                                                                    <div
                                                                ...
                                                                <td className="row-actions text-center">
                                                                    ...
                                                                    <a
                                                                        ...
                                                                        title="Delete"
                                                                        onClick={async () => {
                                                                            let del = window.confirm(`Are you sure you want to delete "${stu.firstname + " " + stu.middlename + " " + stu.surname}"?`);
                                                                            if (del) {
                                                                                await RemoveStudent({
                                                                                    variables: {
                                                                                        id: stu.id,
                        ...
                        <div className="element-wrapper">
                            ...
                            <div className="element-box ">
                                ...
                                <div className="text-center mb-5">
                                    ...
                                    <h2 className="up-header ">{activeStudent.first_name + " " + activeStudent.middle_name + " " + activeStudent.surname}</h2>
                                    <h6 className="up-sub-header text-uppercase">{activeStudent.reg_no}</h6>
                                </div>
                                        ...
                                        <div className="tab-pane active" id="basic-info">
                                            <div className="text-center element-box no-bg no-shadow">
                                                <ul className="pro-details">
                                                    ...
                                                    <li>
                                                        <span>Date of Birth</span> | <b>{CLEAN_DATE(activeStudent.dob)}</b>
                                                        <span className="badge badge-primary ml-2 p-1">{GetAge(activeStudent.dob)}Yrs</span>
                                                    </li>
                                                    <li>
                                                        <span>Date Created</span> | <b>{CLEAN_DATE(activeStudent.created_at)}</b>
                                                    </li>
                                                    <li>
                                                        <span>Level</span> | <b>{activeStudent.current_class?.level?.name}</b>
                                                    </li>
                                                    <li>
                                                        <span>Class</span> | <b>{activeStudent.current_class?.name}</b>
                                                    </li>
                                                    <li>
                                                        <span>State of Origin</span> | <b>{activeStudent.state}</b>
                                                    </li>
                                                    <li>
                                                        <span>LGA</span> | <b>{activeStudent.lga}</b>
                                                    </li>
                                                    <li>
                                                        <span>Address</span> | <b>{activeStudent.address}</b>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                        ...
                                        <div className="tab-pane" id="subjects">
                                            <div className="text-center element-box no-bg no-shadow">
                                                <div className="text-right">
                                                    {/* Subjects */}
                                                    <label
                                                        className={activeStudent?.selected_subjects.length === 0 ? "btn btn-sm btn-secondary" : "badge badge-success-inverted"}
                                                        style={{ padding: "4px 5px" }}
                                                    >
                                                        {activeStudent?.selected_subjects.length === 0 ? "General" : "Selected"}
                                                    </label>
                                                </div>
                                                <SubjectList subjects={activeStudent?.selected_subjects.length > 0 ? activeStudent.selected_subjects : sData?.GetSubjectsForRegistration.docs} />
                                                        ...
                                                        <div className="row">
                                                            ...
                                                            {activeStudent.guardians.length === 0 && (
                                                                <div className="col-12 mb-5">
                                                                    <label className="text-danger">No guardian record found!</label>
                                                                </div>
                                                            )}
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="tab-pane" id="subjectAtt">
                                                        <div className="row">
                                                            <div className="col-md-4">
                                                                <a className="element-box el-tablo no-bg bg-beige mt-0 active-bdr-primary" href="javascript:void(0)">
                                                                    <div className="label">8:30AM - 9:20AM - (Monday)</div>
                                                                    <div className="value">English Language</div>
                                                                </a>
                                                                <a className="element-box el-tablo no-bg bg-beige mt-0" href="javascript:void(0)">
                                                                    <div className="label">8:30AM - 9:20AM - (Monday)</div>
