...
import { GET_SUBJECTS, NEW_SUBJECT, REMOVE_SUBJECT, UPDATE_SUBJECT } from "../../queries/Subject.query";
...
const Subjects: FC<IProps> = ({ history }) => {
    ...
    const [RemoveSubject, { loading: rLoading }] = useMutation(REMOVE_SUBJECT, {
        ...
        update: (cache, { data }) => {
            ...
            const index = q.GetSubjects.docs.findIndex((i: any) => i.id === data.RemoveSubject.doc.id);
    ...
    const [UpdateSubject, { loading: uLoading }] = useMutation(UPDATE_SUBJECT, {
        ...
        update: (cache, { data }) => {
            ...
            const index = q.GetSubjects.docs.findIndex((i: any) => i.id === data.UpdateSubject.doc.id);
                ...
                <div className="col-md-12">
                    <div className="content-i">
                                            ...
                                            <div className="element-box-tp">
                                                <div className="table-responsive">
                                                    <table className="table table-padded">
                                                        ...
                                                        <tbody>
                                                            {data.GetSubjects.docs.map((sub: any, index: number) => (
                                                                <tr>
                                                                    <td>{index + 1}</td>
                                                                    <td>{sub.title}</td>
                                                                    <td>{sub.code}</td>
                                                                    <td>
                                                                        {sub.levels.map((lev: any) => (
                                                                            <span className="badge badge-success-inverted">{lev.name}</span>
                                                                        ))}
                                                                    </td>
                                                                    <td className="row-actions text-center">
                                                                        ...
                                                                        <a
                                                                            ...
                                                                            title="Delete"
                                                                            onClick={async () => {
                                                                                let del = window.confirm(`Are you sure you want to delete "${sub.title + " ( " + sub.code + " )"}"?`);
                                                                                if (del) {
                                                                                    await RemoveSubject({
                                                                                        variables: {
                                                                                            id: sub.id,
                        ...
                        <h2 className="text-danger">No Subject found!</h2>
                                        ...
                                        {data && data.GetSubjects.docs.length === 0 && (
                                            <div className="text-center pt-5 fade-in">
                                                <h3 className="text-danger"> No Subject record found!</h3>
                                            </div>
                                        )}
                                    </div>
                                </div>
