...
import { NEW_GUARDIAN_TYPE, GET_GUARDIAN_TYPES, REMOVE_GUARDIAN_TYPE, UPDATE_GUARDIAN_TYPE } from "../../queries/Guardian.query";
...
const GuardianType: FC<IProps> = ({ history }) => {
    ...
    const [NewGuardianType, { loading: nLoading }] = useMutation(NEW_GUARDIAN_TYPE, {
        onError: (err) => toast.error(err.message),
    ...
    // Remove GuardianType
    const [RemoveGuardianType, { loading: rLoading }] = useMutation(REMOVE_GUARDIAN_TYPE, {
        onError: (err) => toast.error(err.message),
        update: (cache, { data }) => {
            const q: any = cache.readQuery({
                query: GET_GUARDIAN_TYPES,
            ...
            const index = q.GetGuardianTypes.docs.findIndex((i: any) => i.id === data.RemoveGuardianType.doc.id);
    ...
    // Update GuradianType
    const [UpdateGuradianType, { loading: uLoading }] = useMutation(UPDATE_GUARDIAN_TYPE, {
        onError: (err) => toast.error(err.message),
        onCompleted: (data) => {
        ...
        update: (cache, { data }) => {
            ...
            const index = q.GetGuardianTypes.docs.findIndex((i: any) => i.id === data.UpdateGuardianType.doc.id);
                                                    ...
                                                    <div className="col-sm-12 col-md-8 col-lg-10">
                                                        <div className="input-group mb-3">
                                                            ...
                                                            <input
                                                                ...
                                                                onChange={({ currentTarget }) => SetNewGuardianType(currentTarget.value)}
                                                            />
                                                        </div>
                                                    </div>
                                            ...
                                            <div className="table-responsive">
                                                ...
                                                {data && data.GetGuardianTypes.docs.length > 0 && (
                                                    <table className="table table-padded">
                                                        ...
                                                        <tbody>
                                                            {data.GetGuardianTypes.docs.map((type: any, index: number) => (
                                                                <tr>
                                                                    <td>{index + 1}</td>
                                                                    <td>{type.name}</td>
                                                                    <td className="row-actions text-center">
                                                                        ...
                                                                        <a
                                                                            ...
                                                                            title="Delete"
                                                                            onClick={async () => {
                                                                                let del = window.confirm(`Are you sure you want to delete "${type.name}"?`);
                                                                                if (del) {
                                                ...
                                                {data && data.GetGuardianTypes.docs.length === 0 && (
                                                    <div className="text-center pt-5 fade-in">
                                                        <h3 className="text-danger"> No Guardian Type record found!</h3>
                                                    </div>
                                                )}
