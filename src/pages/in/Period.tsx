...
const Period: FC<IProps> = ({ history }) => {
    ...
    const [UpdatePeriod, { loading: uLoading }] = useMutation(UPDATE_PERIOD, {
        ...
        update: (cache, { data }) => {
            ...
            const index = q.GetSchoolPeriodList.docs.findIndex((i: any) => i.id === data.UpdatePeriod.doc.id);
    ...
    const [RemovePeriod, { loading: rLoading }] = useMutation(REMOVE_PERIOD, {
        ...
        update: (cache, { data }) => {
            ...
            const index = q.GetSchoolPeriodList.docs.findIndex((i: any) => i.id === data.RemovePeriod.doc.id);
                            ...
                            <div className="element-wrapper">
                                ...
                                <div className="element-box">
                                    <span className="element-actions">
                                        ...
                                        >
                                            <i className={`os-icon os-icon-chevron-${showNewPeriod ? "down" : "up"} icon-lg`}></i>
                                        </a>
                                    </span>
                                    <h6 className={`element-header ${!showNewPeriod ? "mb-0" : ""}`}>New Period</h6>
                                    {showNewPeriod && (
                                        <form
                                            onSubmit={async (e) => {
                                                e.preventDefault();
                                ...
                                {data?.GetSchoolPeriodList.docs.length > 0 && (
                                    <div className="element-box">
                                        ...
                                        <hr />
                                        <div className="row">
                                            {data.GetSchoolPeriodList.docs.map((prd: any, index: number) => (
                                                <div key={index} className={`col-sm-2 p-3 text-center ${prd.break ? "brk-period" : "period"} period`}>
                                                    <label className="mb-2">
                                                        <strong>{prd.from + " - " + prd.to}</strong>
                                                    </label>
                                                    {prd.break && <h5 className="text-primary">Break</h5>}
                                                    <hr />
                                                    <div className="justify-content-center">
                                                        ...
                                                        <a
                                                            ...
                                                            title="Delete"
                                                            onClick={async () => {
                                                                let del = window.confirm(`Are you sure you want to delete Period "${prd.from + " - " + prd.to}"?`);
                                                                if (del) {
                                                                    await RemovePeriod({
                                                                        variables: {
                                                                            id: prd.id,
