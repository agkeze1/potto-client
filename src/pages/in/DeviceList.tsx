...
import { GET_ASSIGNED_DEVICES, GET_UNASSIGNED_DEVICES, ASSIGN_DEVICE, UNASSIGN_DEVICE } from "../../queries/Device.query";
...
const DeviceList = () => {
    const [showUnassignedDevices, SetShowUnassignedDevices] = useState<boolean>(false);
    ...
    const [AssignDevice, { loading: aLoading }] = useMutation(ASSIGN_DEVICE, {
        ...
        update: (cache, { data }) => {
            ...
            const ind = d.GetUnassignedDevices.docs.findIndex((i: any) => i.id === data.AssignToClass.doc.id);
    ...
    const [UnAssignDevice, { loading: uDLoading }] = useMutation(UNASSIGN_DEVICE, {
        onError: (err) => {
            toast.error(CleanMessage(err.message));
        },
        update: (cache, { data }) => {
            ...
            const ind = d.GetAssignedDevices.docs.findIndex((i: any) => i.id === data.UnAssignDevice.doc.id);
    ...
    return (
        <>
                    ...
                    <div className="content-i">
                        <div className="content-box">
                            <div className="element-wrapper">
                                ...
                                <h5 className="element-header">{showUnassignedDevices ? "Unassigned Device List" : "Assigned Device List"}</h5>
                                <div className="row justify-content-center ">
                                    <div className="col-lg-12">
                                        <div className="element-box-tp">
                                            <LoadingState loading={loading || uLoading || uDLoading} />
                                            ...
                                            {showUnassignedDevices && <Devices devices={uData?.GetUnassignedDevices.docs} type="Unassigned" SetActiveDevice={SetActiveDevice} showAssignBtn={true} />}
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <button id="btnAssignDevice" data-target="#AssignToClassModal" data-toggle="modal" style={{ display: "none" }}></button>

            {/* Device On History Modal */}
            <div aria-hidden="true" className="modal fade" id="PowerHistoryModal" role="dialog">
                        ...
                        <div className="modal-body">
                            <div className="table-responsive element-box no-bg no-shadow">
                                <div className="text-center mb-3">
                                    <i className="os-icon os-icon-smartphone" style={{ fontSize: "100px" }}></i>
                                    <div className="mt-3">
                                ...
                                {activeDevice?.state_histories?.length > 0 && (
                                    <table className="table table-striped text-center">
                                        ...
                                        <tbody>
                                            {activeDevice?.state_histories?.map((his: any, idx: number) => (
                                                <tr>
                                                    <td>{idx + 1}</td>
                                                    <td>
                                                        <span className="text-primary">{cleanDate(his.date)}</span>
                                                    </td>
                                                </tr>
                                            ))}
                                        </tbody>
                                ...
                                {activeDevice?.state_histories?.length === 0 && <h6 className="text-danger mt-3">No "On History" found!</h6>}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div aria-hidden="true" className="modal fade" id="AssignToClassModal" role="dialog">
                                ...
                                <div className="row">
                                    ...
                                    <div className="col-12 mb-4">
                                        ...
                                        <Select
                                            options={classes}
                                            value={{
                                                label: activeClass?.name || <span className="text-gray">Select...</span>,
