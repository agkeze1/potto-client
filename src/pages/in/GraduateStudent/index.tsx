...
const GraduateStudent = () => {
  ...
  const isStudentIn = (item: any): boolean => {
    const index = dueStudents.findIndex(
      (c: any) => c.id.toString() === item.id.toString()
    );
    return index !== -1;
  };
              ...
              {excludedStudents.length > 0 && (
                <div className="fade-in">
                  ...
                  <StudentCheckExemptedList
                    items={excludedStudents}
                    onCheck={(item: any) => moveStudent(item)}
                  />
                </div>
              )}
            </div>
              ...
              <>
                <div className="fade-in">
                  <div className="element-box no-bg bg-white">
                    <label htmlFor="offLevel">Select students to exempt</label>
                    <Select
                      id="offLevel"
                      isMulti={false}
                      options={dueStudents.map((i: any) => ({
                        value: i.id,
                        label: i.full_name,
                      }))}
                      onChange={(item: any) => {
                        const _item = dueStudents.find(
