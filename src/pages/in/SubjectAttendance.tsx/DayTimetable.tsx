...
const DayTimetable: FC<IProps> = ({ level, _class }) => {
    ...
    const [GetTimetableAttendance, { loading: ttALoading }] = useLazyQuery(TIMETABLE_ATTENDANCE, {
        onError: (err) => toast.error(CleanMessage(err.message)),
        onCompleted: (data) => {
            ...
            {data && (
                <>
                                    ...
                                    <div className="col-md-4">
                                        <div className="element-box no-bg bg-white">
                                            ...
                                            {data.GetClassTimetable.docs.map((item: any, idx: number) => (
                                                <a
                                                    key={idx}
                                                    className={`el-tablo att-date-crd ${activeRecord?.day === item.day ? "active-att-date" : ""}`}
                                                ...
                                                >
                                                    <div className="row">
                                                        <div className="col-10">
                                                            <div className="text-primary">{days.days.find((i: any) => i.value === item.day)?.label.toUpperCase()}</div>
                                                        </div>
                                                    </div>
                                                </a>
                                            ))}
                                        ...
                                        <div className="col-md-8">
                                            <div className="element-box no-bg bg-white">
                                                <h6 className="element-header">
                                                    Timetable <b className="text-primary">( {days.days.find((i: any) => i.value === activeRecord.day)?.label.toUpperCase()} )</b>
                                                </h6>
                                                {activeRecord.timetable_list.length > 0 && (
                                                    <div className="row">
                                                        {activeRecord.timetable_list?.map((tt: any, index: number) => (
                                                            <div className="col-4">
                                                                <a
                                                                    key={index}
                                                                ...
                                                                >
                                                                    <h6 className="label element-header">
                                                                        ...
                                                                        {clickedTimetablee?.id === tt.id && <LoadingState loading={ttALoading} />}
                                                                    </h6>

                                                                    <label className="label" style={{ marginTop: "-20px" }}>
                                                                        {tt.teacher?.name}
                                                                    </label>
                                                                    <div className="value text-primary">{tt.subject?.code}</div>
                                                                </a>
                                                            </div>
                                                        ))}
                                                    </div>
                                                ...
                                                {activeRecord.timetable_list.length === 0 && (
                                                    <div className="col-12">
                                                        <h6 className="text-danger text-center">No Timetable record found!</h6>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                    ...
                    {data.GetClassTimetable.docs.length === 0 && (
                        <>
                            <div className="col-12">
                                <h5 className="text-danger text-center mt-5">No Timetable record found for selected class!</h5>
                            </div>
                        </>
                    )}
